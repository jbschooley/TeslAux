...
**Also built: read pacing.** Track data is served at **1.1x real time** after a
two-second burst; metadata is never paced, since throttling the FAT and the
directory would make every wake slow.

This is what makes pause detection possible rather than merely faster. Served at
full speed the car filled its buffer, stopped reading for seconds, and played
from memory — so *reads stopping* meant nothing, and the LED flapped between
"playing" and "paused" throughout normal playback. Held to barely above real
time the car can never get ahead, so it reads about three times a second and a
pause shows up within one read.

The two-second burst exists so a player can prime its buffer without stuttering
at the start of a track; it is small enough that a pause cannot hide behind it.

**Not built:**

1. **A Consumer Control HID interface on the source side**, to send the media
