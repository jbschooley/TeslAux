...
async fn capture(
    ...
    loop {
        ...
        //
        // The DMA moves whole blocks of samples, but nothing ties a block
        // boundary to a *frame* boundary. The state machine runs from the
        // moment it is enabled while the first DMA is not armed until this task
        // first runs, so samples accumulate in the RX FIFO; if an odd number are
        ...
        // because the program's first instruction waits for LRCK low. Clearing
        // the FIFO discards whatever was mid-frame.
        if !have_work {
            sm.set_enable(false);
            sm.clear_fifos();
            sm.restart();
